...
void drich_hepmc_writer_sp(int n_events, int etaBinID, float phi_bin, double mom, int pdg)
{
...
//const char * out_fname = "out.hepmc";
  ...
  if(pdg>0)
  oss << pdg << "_" << mom << "_" << etaBinID<<"_" << phi_bin<<".hepmc";
  else
  oss <<"m"<<pdg << "_" << mom << "_" << etaBinID<<"_" << phi_bin<<".hepmc";
  ...
  TRandom *rdmn_gen = new TRandom(seed);
    ...
    // evt.set_event_number(events_parsed); 
      ...
      for(int iq=0; iq<1; iq++){ // daughter particle loop  
        ...
        Double_t phi   = M_PI*phi_bin*0.25; // phi_multiplier should be between 0 and 2
